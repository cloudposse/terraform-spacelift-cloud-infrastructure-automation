...
| <a name="input_policies_enabled_map"></a> [policies\_enabled\_map](#input\_policies\_enabled\_map) | Map of default policies to attach to all Spacelift stacks | `map(string)` | <pre>{<br>  "git_push.proposed-run": {<br>    "labels": [],<br>    "name": "GIT_PUSH Proposed Run Policy",<br>    "type": "GIT_PUSH"<br>  },<br>  "git_push.tracked-run": {<br>    "labels": [],<br>    "name": "GIT_PUSH Tracked Run Policy",<br>    "type": "GIT_PUSH"<br>  },<br>  "plan.default": {<br>    "labels": [],<br>    "name": "PLAN Default Policy",<br>    "type": "PLAN"<br>  },<br>  "trigger.dependencies": {<br>    "labels": [],<br>    "name": "TRIGGER Dependencies Policy",<br>    "type": "TRIGGER"<br>  }<br>}</pre> | no |
