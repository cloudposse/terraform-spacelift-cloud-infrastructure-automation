...
| <a name="input_commit_sha"></a> [commit\_sha](#input\_commit\_sha) | The commit SHA for which to trigger a run. Requires `var.spacelift_run` to be enabled. | `string` | `null` | no |
...
| <a name="input_spacelift_run"></a> [spacelift\_run](#input\_spacelift\_run) | Whether to create a `spacelift_run` resource. | `bool` | `false` | no |
