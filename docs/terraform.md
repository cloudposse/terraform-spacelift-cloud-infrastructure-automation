...
| <a name="input_context_filters"></a> [context\_filters](#input\_context\_filters) | Context filters to only create stacks for specific environments, tenants and stages | `map(list(string))` | <pre>{<br>  "environments": [],<br>  "stages": [],<br>  "tenants": []<br>}</pre> | no |
