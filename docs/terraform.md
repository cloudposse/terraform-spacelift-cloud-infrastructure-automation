...
| Name | Source | Version |
|------|--------|---------|
| <a name="module_spacelift_config"></a> [spacelift\_config](#module\_spacelift\_config) | cloudposse/stack-config/yaml//modules/spacelift | 0.22.0 |
...
| <a name="input_policies_available_map"></a> [policies\_available\_map](#input\_policies\_available\_map) | Map of available default policies to create in Spacelift (these policies will not be attached to Spacelift stacks by default, use `var.policies_enabled`) | `map(string)` | <pre>{<br>  "git_push.proposed-run": {<br>    "labels": [],<br>    "name": "GIT_PUSH Proposed Run Policy",<br>    "type": "GIT_PUSH"<br>  },<br>  "git_push.tracked-run": {<br>    "labels": [],<br>    "name": "GIT_PUSH Tracked Run Policy",<br>    "type": "GIT_PUSH"<br>  },<br>  "plan.default": {<br>    "labels": [],<br>    "name": "PLAN Default Policy",<br>    "type": "PLAN"<br>  },<br>  "trigger.dependencies": {<br>    "labels": [],<br>    "name": "TRIGGER Dependencies Policy",<br>    "type": "TRIGGER"<br>  }<br>}</pre> | no |
...
| <a name="input_policies_by_map_enabled"></a> [policies\_by\_map\_enabled](#input\_policies\_by\_map\_enabled) | Map of existing policy names to attach to all Spacelift stacks. These policies must be created outside of this module | `map(string)` | `{}` | no |
