...
| <a name="input_context_filters"></a> [context\_filters](#input\_context\_filters) | Filter stacks using `environments`, `tenants`, or `stages`. | `map(list(string))` | <pre>{<br>  "environments": [],<br>  "stages": [],<br>  "tenants": []<br>}</pre> | no |
...
| <a name="input_spacelift_stacks_override"></a> [spacelift\_stacks\_override](#input\_spacelift\_stacks\_override) | A map of Spacelift stacks to override the stacks in the module | `any` | `{}` | no |
